...
exports.run = async (client, msg, args) => {
  ...
  ctx.fill()
        ...
        var desc = db.fetch(`desc_${u.id}`) || "Düzenlemek için //açıklama"
  ...
  var sirketisim = db.fetch(`sirketisim_${u.id + msg.guild.id}}`)
   let sirkettipi = db.fetch(`sirkettipi_${u.id+ msg.guild.id}`);
        ...
        ctx.arc(257 + 18.5, 147.5 + 18.5 + 36.25, 18.5, 1.5 * Math.PI, 0.5 * Math.PI, true);
        ctx.fill();
        ctx.fillRect(257 + 18.5, 147.5 + 36.25, xp * de2  , 37.5);
        ctx.arc(257 + 18.5 + xp * de2, 147.5 + 18.5 + 36.25, 18.75, 1.5 * Math.PI, 0.5 * Math.PI, false);
        ctx.fill();
        ctx.fillStyle = `#${re}`;
        ctx.font = '28px Impact';
        ctx.textAlign = "right";
        ctx.fillText(`Sıralama #${sira} | Seviye ${lvl || 0} | ${para || 0}₺ `, 670, 70);
        ctx.font = '20px Impact';
        ctx.textAlign = "right";
        ctx.fillText(`${xp || 0} / ${lvl*25} XP`, 670, 100);
  ctx.fillStyle = `#bfbfbf`;
  ctx.font = '25px Impact';
        ctx.fillStyle = `#${re}`;
        ctx.textAlign = "left";
        ctx.fillText(`${desc}`, 270, 155)
        ctx.font = '25px Impact';
        ctx.fillStyle = `#${re}`;
        ctx.textAlign = "center";
        ctx.fillText(`${sirkettipi || "Şirketi yok"} `, 141, 265)
        ctx.beginPath();
        ctx.lineWidth = 8;
  ctx.fill()
        ctx.lineWidth = 8;
...
exports.help = {
  ...
  usage: 'profil [@kullanıcı/renk/resim] [renk kodu/resim URLsi/sıfırla]'
}; 