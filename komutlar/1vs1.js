...
exports.run = async (client, message, args) => {
  ...
  if (opponent.id === '60912456873550351') return message.reply('Botlarla 1vs1 atamazsın yenen belli çünkü.');
		...
		try {
			...
			while (userHP > 0 && oppoHP > 0) { // eslint-disable-line no-unmodified-loop-condition
				...
				if (choice === 'saldır') {
					...
					await message.channel.send(`${user}, kendisini kalkan ile savundu!`);
					guard = true;
					reset(false);
				} else if (choice === 'ulti') {
					...
					if (!miss) {
						...
						await message.channel.send(`${user}, Çoook uzak galaksilerden gelen ultra sonik enerjiyi yeterli miktarda topladın ve **${damage}** hasar vurdun!!`);
						dealDamage(damage);
					} else {
						await message.channel.send(`${user}, Ultra gücünü kullanamadın, daha fazla odaklanman gerekli!`);
					}
					reset();
				} else if (choice === 'kaç') {
					await message.channel.send(`${user}, bir hayalet misali arenadan kayboldu :)`);
					forfeit();
					break;
				} else {
